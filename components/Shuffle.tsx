...
const Shuffle: React.FC<ShuffleProps> = ({
  ...
  useGSAP(
    () => {
      ...
      // FORCE REFLOW: Memastikan font terbaca sebelum pengukuran
      ...
      const build = () => {
        ...
        chars.forEach(ch => {
          ...
          // Pastikan font diterapkan saat pengukuran
          ch.style.fontFamily = fontFamily;

          const w = ch.getBoundingClientRect().width;
          // if (!w) return; // Removed safety check that might skip spaces

          const wrap = document.createElement('span');
          wrap.className = 'inline-block overflow-hidden align-baseline text-left';
          // Set width awal (penting untuk animasi)
      ...
      const cleanupToStill = () => {
        wrappersRef.current.forEach(w => {
          // FIX UTAMA: Reset width wrapper jadi auto agar spasi kembali normal
          w.style.width = 'auto';
      ...
      const play = () => {
        ...
        const tl = gsap.timeline({
          ...
          onComplete: () => {
            playingRef.current = false;
            if (!loop) {
              cleanupToStill(); // Ini akan memperbaiki spasi di akhir animasi
